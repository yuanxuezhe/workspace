...
void CThread::Create( bool detached, bool sscope )
{
    ...
    if( !m_IsRunning )
    {
        ...
        {
            std::cout << "(CThread) create : pthread_attr_init ("
        ...
        if ( detached )
	{
		 ...
		 PTHREAD_CREATE_DETACHED)) != 0)
            {
                std::cout << "(CThread) create : pthread_attr_setdetachstate("
                     << strerror( status ) << ")" << std::endl;
                return;
            }
        }
        ...
        if ( sscope )
        {
		 ...
		 PTHREAD_SCOPE_SYSTEM)) != 0 )
            {
                std::cout << "(CThread) create : pthread_attr_setscope ("
                     << strerror( status ) << ")" << std::endl;
                return;
            }
        }
	...
	{
           std::cout << "(CThread) create : pthread_create ("
                << strerror( status ) << ")" << std::endl;
	}
       else
           m_IsRunning = true;
        ...
        std::cerr << "(CThread) create : thread is already running" << std::endl;
}
...
void CThread::Detach( )
{
    if( m_IsRunning )
    {
        ...
        if ((status = pthread_detach( m_ThreadId )) != 0)
            std::cout << "(CThread) detach : pthread_detach ("
                 << strerror( status ) << ")" << std::endl;
    }
}
...
void CThread::WaitForOver( )
{
    if ( m_IsRunning )
    {
        ...
        if ((status = pthread_join( m_ThreadId, NULL )) != 0)
            std::cout << "(CThread) join : pthread_join ("
                 << strerror( status ) << ")" << std::endl;
...
void CThread::Cancel ()
{
    if ( m_IsRunning )
    {
        ...
        if ((status = pthread_cancel( m_ThreadId )) != 0)
            std::cout << "(CThread) cancel : pthread_cancel ("
                 << strerror( status ) << ")" << std::endl;
    }
}
...
void CThread::SendSignalToThread ( int signo )
{
    if ( m_IsRunning )
    {
        ...
        if ((status = pthread_kill( m_ThreadId, signo )) != 0)
            std::cout << "(CThread) kill : pthread_kill ("
                 << strerror( status ) << ")" << std::endl;
        else
            m_IsRunning = false;
    }
...
void CThread::SetToSleep ( long m_sec, long nano_sec )
{
    if ( m_IsRunning )
    {
        ...
        if (nanosleep( &interval, 0 ) != 0)
            std::cout << "(CThread) delay : error in nanosleep" << std::endl;
    }
}
